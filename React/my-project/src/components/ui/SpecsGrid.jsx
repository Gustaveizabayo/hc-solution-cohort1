...
const SpecsGrid = () => (
  <section id="specs" className="max-w-8xl mx-auto px-8 py-16 md:py-24">
    <div className="text-center max-w-4xl mx-auto mb-16 flex flex-col items-center">
      <h2 className="serif text-5xl md:text-[76px] text-gray-900 mb-6 font-medium">Why Choose Area?</h2>
      <p className="text-[24px] font-medium text-gray-400 mb-8 max-w-[600px] leading-relaxed">
        You need a solution that keeps up. That's why we developed Area. A developer-friendly approach to streamline your business.
      </p>
      <button className="bg-[#e4ebce] text-[#1B2921] px-8 py-3 rounded-full font-bold text-[24px] tracking-wide hover:bg-[#d6dea4] transition">
        Discover More
      </button>
    </div>

    <div className="relative w-full mx-auto mt-16 text-left overflow-hidden">
      <div className="absolute left-0 top-0 bottom-0 w-1/3 bg-gray-50/50 border-r border-gray-200/80 z-0" />

      <div className="relative z-10 grid grid-cols-3 w-full items-center">
        
        <div className="text-center py-8">
          <span className="text-[40px] font-bold text-gray-900 tracking-tight">Area</span>
        </div>
        <div className="text-center py-8">
          <span className="text-[40px] font-serif text-gray-500 tracking-wide">WebSurge</span>
        </div>
        <div className="text-center py-8">
          <span className="text-[40px] font-light font-sans text-gray-500">HyperView</span>
        </div>

        {rows.map((row, i) => (
          <React.Fragment key={i}>
            <div className="col-span-3 border-t border-gray-100" />
            <div className="py-7 px-6 lg:px-12 text-[16px] font-mono text-gray-800 flex items-center gap-3">
              <span className="text-gray-400/80 text-xs font-sans">✓</span> {row.label}
            </div>
            <div className="py-7 px-6 lg:px-12 text-[16px] font-mono text-gray-600 flex items-center gap-3">
